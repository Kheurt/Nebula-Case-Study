...
- [ ] T034 [US1] Implement programs catalog page in `src/app/(public)/programs/page.tsx`: lists published programs, domain filter UI (enum select), coach filter UI (search-select populated from coach names — derive unique coaches client-side from the programs list returned by `getPublishedPrograms`, no additional server action required), text search field, empty state with CTA
