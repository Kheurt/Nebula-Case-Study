...
export interface CoachExplorationRow {
  id: string;
  title: string;
  description: string;
  dueDate: Date | null;
...
export async function getCoachExplorations(): Promise<ActionResult<CoachExplorationRow[]>> {
  ...
  try {
    const explorations = await prisma.exploration.findMany({
      where: {
        OR: [
          { program: { coachId } },
          { session: { cohort: { program: { coachId } } } },
        ],
      },
      include: {
    ...
    const rows: CoachExplorationRow[] = explorations.map((e) => ({
      id: e.id,
      title: e.title,
    ...
    logger.error('getCoachExplorations error', { coachId, error: String(err) });
